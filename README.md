...
Taken down after the contest ended.

~~You can try out the bot here: https://t.me/bh11412_test_bot. The idea is that each community that wants to use the bot deploys its own instance of it.~~

~~**Note to contest reviewers**: please contact https://t.me/boyMCMXCII if, for some reason, the server becomes not responsive - we'll start it up straight away. Telegram servers sometimes fail to return images, in that case you'll see fallback placeholders.~~
...
Clone this repo (**`public` branch**) into your local directory (run `git clone https://github.com/anton-novak/community-board-bot.git`), navigate to both `server` and `web-app` directories in your OS terminal and run `npm i` there to install back-end and front-end dependencies. 
...
To start both the back end and the front end you can use the pre-made start-up scripts. Navigate to the top directory of the repo in your OS terminal, run `npm i` to download `concurrently` package and run `npm run start:win32` on Windows or `npm run start:unix` on UNIX OS (*UNIX scripts not tested*). 

If the general start-up script fails, or you want to run things in separate terminals, you can do the following:
* Navigate to `/server` directory and start the back end with `npm run start_server` command in your OS terminal. 
* To start the front-end server run `($env:HTTPS = "true") -and (npm run start_webapp)` from `/web-app` directory in Windows PowerShell or `HTTPS=true npm run start_webapp` in a UNIX OS terminal (Create React App [docs page](https://create-react-app.dev/docs/using-https-in-development) on that). After the front-end server starts, the script will open a web app page in your browser that should show `Page not found` - this is normal, the app is only supposed to work with the bot.
...
```
App: routes
    AdBrowser || Office: main views containers, they maintain category and sorting states generated by NavBar
        NavBar
        ItemContainer: ads container that maintains ads data in its state
            ItemCard: ad card with thumbnail image within the container list
                Notification
            ItemModal: scrollable ad drawer with larger image and save button
                Notification
    NotFound
```
...
* We have not found an easy way to test the bot and the web app on a mobile device. This became possible closer to the submission deadline after deploying the bot, and required purchasing a virtual server, domain name and SSL certificate to set up HTTPS protocol and handling the deployment. A feature got commented out as a result as it did not behave as expected on a mobile device (fade out on modal overflow to let user know that there's more content down below, substituted with drawer pull-out chevron icon on top).
...
[Anton](https://t.me/boyMCMXCII) & [Alexander](https://t.me/newversionofkelvin) Novak, for 22 September 2023 Telegram Mini-App Contest submission.